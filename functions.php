...
/* Create the core pages automatically when the theme is activated on a new WordPress install. */
function matin_parto_create_core_pages() {
    $pages = array(
        'about' => array( 'title' => 'درباره من', 'slug' => 'about' ),
        'contact' => array( 'title' => 'تماس با ما', 'slug' => 'contact' ),
    );
    ...
    /* Make WordPress regenerate pretty-page routes after the pages are created. */
    flush_rewrite_rules();
}
add_action( 'after_switch_theme', 'matin_parto_create_core_pages' );

/* Also repair missing pages on an already-active install, without creating duplicates. */
function matin_parto_ensure_core_pages() {
    if ( get_option( 'matin_parto_core_pages_ready' ) === '1' ) return;
    matin_parto_create_core_pages();
    update_option( 'matin_parto_core_pages_ready', '1' );
}
add_action( 'init', 'matin_parto_ensure_core_pages', 20 );
...
function matin_parto_enqueue_assets() {
    ...
    if ( $video_image ) $custom_css .= '.mp-video-thumb,.mp-video-mini{background-image:url("' . $video_image . '") !important;}';
    if ( $course_image ) $custom_css .= '.mp-course-image{background-image:url("' . $course_image . '") !important;}';
    if ( $cta_image ) {
    ...
    if ( $backdrop ) $custom_css .= '.mp-hero__visual:before{content:var(--mp-hero-backdrop-text);position:absolute;z-index:1;right:2%;top:4%;font-family:Georgia,serif;font-size:clamp(38px,6vw,82px);font-weight:700;letter-spacing:3px;line-height:.9;color:rgba(60,48,51,.055);white-space:pre-line;pointer-events:none;}';
    wp_add_inline_style( 'matin-parto-home', $custom_css );

    $js = $base_dir . '/assets/js/main.js';
    if ( file_exists( $js ) ) wp_enqueue_script( 'matin-parto-main', $base_uri . '/assets/js/main.js', array(), (string) filemtime( $js ), true );
}
...
function matin_parto_customize_register( $wp_customize ) {
    $wp_customize->add_section( 'matin_parto_home', array( 'title' => 'صفحه اصلی و تصاویر', 'priority' => 150, 'description' => 'تصاویر صفحه اصلی و بخش پایانی را از اینجا انتخاب کنید.' ) );
    ...
    foreach ( $image_controls as $setting_id => $labels ) {
        $wp_customize->add_setting( $setting_id, array( 'default' => '', 'sanitize_callback' => 'esc_url_raw', 'transport' => 'refresh' ) );
        $wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, $setting_id, array( 'label' => $labels[0], 'description' => $labels[1], 'section' => 'matin_parto_home' ) ) );
    }
    $text_controls = array(
    ...
    foreach ( $text_controls as $setting_id => $control ) {
        $wp_customize->add_setting( $setting_id, array( 'default' => $control[2], 'sanitize_callback' => 'sanitize_text_field', 'transport' => 'refresh' ) );
        $wp_customize->add_control( $setting_id, array( 'label' => $control[0], 'description' => $control[1], 'section' => 'matin_parto_home', 'type' => 'text' ) );
    }
    $social_controls = array( 'matin_parto_social_whatsapp' => 'لینک واتساپ', 'matin_parto_social_telegram' => 'لینک تلگرام', 'matin_parto_social_instagram' => 'لینک اینستاگرام', 'matin_parto_social_youtube' => 'لینک یوتیوب' );
    foreach ( $social_controls as $setting_id => $label ) {
        $wp_customize->add_setting( $setting_id, array( 'default' => '', 'sanitize_callback' => 'esc_url_raw', 'transport' => 'refresh' ) );
        $wp_customize->add_control( $setting_id, array( 'label' => $label, 'description' => 'لینک کامل شبکه اجتماعی را وارد کنید.', 'section' => 'matin_parto_home', 'type' => 'url' ) );
    }
    $wp_customize->add_section( 'matin_parto_footer', array( 'title' => 'فوتر و پایین سایت', 'priority' => 160, 'description' => 'محتوای ستون‌های اصلی فوتر را از بخش ابزارک‌ها مدیریت کنید.' ) );
...
function matin_parto_fallback_menu() {
    ...
    echo '<li><a href="' . esc_url( home_url( '/#courses' ) ) . '">دوره ها</a></li>';
    echo '<li><a href="' . esc_url( $about ? get_permalink( $about ) : home_url( '/about/' ) ) . '">درباره من</a></li>';
    echo '<li><a href="' . esc_url( home_url( '/#videos' ) ) . '">ویدیو های آموزشی</a></li>';
    echo '<li><a href="' . esc_url( home_url( '/#blog' ) ) . '">وبلاگ</a></li>';
    echo '<li><a href="' . esc_url( $contact ? get_permalink( $contact ) : home_url( '/contact/' ) ) . '">تماس با ما</a></li>';
    echo '</ul>';
}
