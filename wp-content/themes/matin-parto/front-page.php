...
<main class="mp-home">
        ...
        <div class="mp-container mp-hero__grid">
            <div class="mp-hero__copy">
                ...
                <div class="mp-hero__buttons">
                    ...
                    <a class="mp-button mp-button--outline" href="#about-teacher">درباره من</a>
                </div>
                <div class="mp-students" aria-label="بیش از ده هزار زبان‌آموز">
                    <span class="mp-avatars" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i></span>
                    <strong>+10K</strong><span>زبان‌آموز موفق</span>
                </div>
            </div>
            ...
            <div class="mp-hero__visual" aria-label="تصویر مدرس">
                ...
                <?php if ( $hero_image ) : ?>
                    <img class="mp-hero__image" src="<?php echo esc_url( $hero_image ); ?>" alt="مدرس زبان انگلیسی" loading="eager" fetchpriority="high">
                <?php else : ?>
                    <div class="mp-hero__portrait" aria-hidden="true"><span>MATIN<br>PARTO</span></div>
                <?php endif; ?>
    ...
    <section class="mp-benefits mp-container" aria-label="مزیت‌های آموزش">
        <?php foreach ( $benefits as $benefit ) : ?>
            <div class="mp-benefit">
                <span class="mp-benefit__icon mp-icon--<?php echo esc_attr( $benefit[0] ); ?>" aria-hidden="true"></span>
    ...
    <section class="mp-video-section mp-section" id="videos">
        <div class="mp-container">
            ...
            <div class="mp-video-grid">
                <article class="mp-video-feature">
                    <div class="mp-video-thumb <?php echo $hero_image ? 'has-image' : ''; ?>" <?php echo $hero_image ? 'style="background-image:url(' . esc_url( $hero_image ) . ')"' : ''; ?>>
                        <span class="mp-video-number">#18</span><button class="mp-play" type="button" aria-label="پخش ویدئو">▶</button>
                        <strong>How much do you make?</strong><small>سطح متوسط</small><em>◷ 12:45</em>
                    </div>
                </article>
                <div class="mp-video-list">
                    <?php foreach ( $videos as $video ) : ?>
                        <article class="mp-video-row">
                            <div class="mp-video-mini <?php echo $hero_image ? 'has-image' : ''; ?>" <?php echo $hero_image ? 'style="background-image:url(' . esc_url( $hero_image ) . ')"' : ''; ?>><span><?php echo esc_html( $video[2] ); ?></span></div>
                            <div><b><?php echo esc_html( $video[0] ); ?></b><small><?php echo esc_html( $video[1] ); ?></small></div><em><?php echo esc_html( $video[3] ); ?></em>
                        </article>
                    <?php endforeach; ?>
                </div>
            </div>
    ...
    <section class="mp-courses mp-section" id="courses">
        <div class="mp-container">
            <div class="mp-section-head"><a class="mp-circle-arrow" href="#courses" aria-label="حرکت بین دوره‌ها">‹</a><h2>دسته‌بندی دوره‌ها</h2></div>
            <div class="mp-course-carousel">
                <?php foreach ( $categories as $category ) : ?>
                    <article class="mp-course-card">
                        <div class="mp-course-image <?php echo $hero_image ? 'has-image' : ''; ?>" <?php echo $hero_image ? 'style="background-image:url(' . esc_url( $hero_image ) . ')"' : ''; ?>><span><?php echo $hero_image ? '' : 'MATIN PARTO'; ?></span></div>
                        <div class="mp-course-info"><h3><?php echo esc_html( $category[0] ); ?></h3><b><?php echo esc_html( $category[1] ); ?></b><small><?php echo esc_html( $category[2] ); ?></small></div>
                    </article>
                <?php endforeach; ?>
            </div>
        </div>
    ...
    <section class="mp-reviews mp-section" id="reviews">
        <div class="mp-container">
            ...
            <div class="mp-review-grid">
                <?php foreach ( $reviews as $review ) : ?>
                    <article class="mp-review-card">
                        <div class="mp-stars" aria-label="۵ ستاره"><?php echo str_repeat( '★', (int) $review[3] ); ?></div>
                        <p><?php echo esc_html( $review[2] ); ?></p>
                        <div class="mp-review-author"><span></span><div><b><?php echo esc_html( $review[0] ); ?></b><small><?php echo esc_html( $review[1] ); ?></small></div></div>
                    </article>
                <?php endforeach; ?>
            </div>
        </div>
    ...
    <section class="mp-home-cta mp-container">
        <div class="mp-home-cta__person <?php echo $cta_image ? 'has-image' : ''; ?>" <?php echo $cta_image ? 'style="background-image:url(' . esc_url( $cta_image ) . ')"' : ''; ?> aria-hidden="true"></div>
    ...
    <section class="mp-home-about mp-container" id="about-teacher">
        <div><span class="mp-kicker">مسیر یادگیری شما</span><h2>یادگیری زبان را ساده و کاربردی شروع کنید.</h2></div>
        <p>ساختار سایت برای فروش دوره، ویدئوهای آموزشی و محتوای وبلاگ طراحی شده و در مرحله بعد به Tutor LMS متصل می‌شود تا خرید و دسترسی دوره‌ها برای هر کاربر به‌صورت دائمی مدیریت شود.</p>
    </section>
</main>
<?php get_footer(); ?>
