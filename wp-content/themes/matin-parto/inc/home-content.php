...
/* منوی کاملاً مستقل در ستون کناری پیشخوان. */
function matin_parto_testimonials_admin_menu() {
    add_menu_page(
        'نظرات زبان‌آموزان',
        'نظرات زبان‌آموزان',
        'edit_posts',
        'matin-parto-testimonials',
        'matin_parto_testimonials_admin_page',
...
function matin_parto_testimonials_admin_page() {
    if ( ! current_user_can( 'edit_posts' ) ) {
        wp_die( esc_html__( 'دسترسی کافی ندارید.' ) );
    }
...
function matin_parto_social_links_html( $class = '' ) {
    ...
    foreach ( $socials as $key => $label ) {
        ...
        if ( ! $url ) continue;
        echo '<a class="mp-home-social-link mp-home-social-link--' . esc_attr( $key ) . '" href="' . esc_url( $url ) . '" target="_blank" rel="noopener noreferrer" aria-label="' . esc_attr( $label ) . '">' . matin_parto_social_icon( $key ) . '<span>' . esc_html( $label ) . '</span></a>';
    }
    echo '</div>';
}
